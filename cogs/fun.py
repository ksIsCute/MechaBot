...
def setup(client) -> Cog:
  ...
  @fun.command(description="Give someone a pat!", aliases=["givepat", "patmember", "patuser", "userpat", "pat"])
  ...
  @fun.command(description="Are you gay or no?", aliases=["howgay", "gay", "gayrate", "amigay", "gaypercent", "gayamount"])
  ...
  async def _8ball(ctx, *, question):
    ...
    embed = voltage.SendableEmbed(title=f"{ctx.author.name}", icon_url=ctx.author.avatar.url, description=f"""My response to `{str(question)}`...\n `{random.choice(responses)}`!""", color="#516BF2")
    await ctx.send(content="[]()", embed=embed)
  ...
  @fun.command(description="Get some cute fox pics!", aliases=["foxies", "foxs", "foxes", "foxypic", "foxpic", "catpicture", "fox"])
  async def fox(ctx):
    async with aiohttp.ClientSession() as session:
      ...
      embed = voltage.SendableEmbed(title="what sound do foxes make?!", media=catjson['link'], colour="#516BF2", description=factjson['fact'])
      await ctx.send(content="[]()", embed=embed)
  ...
  @fun.command(description="Get some randomly generated animal pics!", aliases=["animals", "animal", "animalpic", "animalz", "petpic"])
